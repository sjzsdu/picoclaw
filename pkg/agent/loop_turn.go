...
func (al *AgentLoop) runTurn(ctx context.Context, ts *turnState) (turnResult, error) {
	...
	var publishedPicoReasoning bool
	var publishedPicoVisibleOutput bool
...
turnLoop:
	for ts.currentIteration() < ts.agent.MaxIterations || len(pendingMessages) > 0 || func() bool {
		...
		if ts.channel == "pico" {
			publishedPicoReasoning = strings.TrimSpace(reasoningContent) != ""
			if publishedPicoReasoning {
		...
		if al.bus != nil && ts.channel == "pico" && len(response.ToolCalls) > 0 && ts.opts.AllowInterimPicoPublish {
			// Intentionally suppress interim assistant narration for pico tool-call turns.
			// The web chat should not show transient content like "I'll fetch that" while tools run.
		}
		...
		if len(response.ToolCalls) == 0 || gracefulTerminal {
			...
			if !ts.opts.SkipSessionPersistence && (strings.TrimSpace(responseContent) != "" || strings.TrimSpace(reasoningContent) != "") {
