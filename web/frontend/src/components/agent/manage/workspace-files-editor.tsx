...
export function WorkspaceFilesEditor({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[calc(100dvh-1.5rem)] w-[calc(100vw-1rem)] max-w-[min(calc(100vw-1rem),120rem)] flex-col gap-0 overflow-hidden rounded-2xl p-0 sm:h-[calc(100dvh-3rem)] sm:w-[calc(100vw-3rem)]">
        <DialogHeader className="border-border/70 bg-muted/30 shrink-0 border-b px-4 py-3 sm:px-5 sm:py-4">
          <DialogTitle className="flex items-center gap-2 text-base">
            <IconFolder className="size-4" />
            {t("pages.agent.manage.workspaceFiles.title")}
          </DialogTitle>
          <DialogDescription className="text-xs">
            {workspaceLabel} · {fileCountLabel}
          </DialogDescription>
        </DialogHeader>

        <div className="grid min-h-0 flex-1 overflow-hidden sm:grid-cols-[280px_minmax(0,1fr)] lg:grid-cols-[320px_minmax(0,1fr)] xl:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="bg-muted/20 border-border/70 flex min-h-0 min-w-0 flex-col border-b sm:border-r sm:border-b-0">
            <div className="border-border/60 shrink-0 space-y-2 border-b px-3 py-3 sm:px-4 sm:py-4">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder={t(
                    "pages.agent.manage.workspaceFiles.searchPlaceholder",
                  )}
                ...
                <div className="space-y-1">
                  {filteredFiles.map((file: WorkspaceFileInfo) => {
                    ...
                    const fileDir = file.path.includes("/")
                      ? file.path.slice(
                          0,
                          file.path.length - fileName.length - 1,
                        )
                      : ""
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isActive
                            ? "border-primary/35 bg-primary/8 ring-primary/10 shadow-sm ring-1"
                            : "bg-background hover:border-border hover:bg-accent/40 border-transparent",
                        )}
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-1.5">
                              ...
                              >
                                {file.can_edit ? (
                                  <IconFilePencil className="size-2.5" />
                                ) : (
                                  <IconFile className="size-2.5" />
                                )}
                              </span>
                            </div>
          ...
          <section className="bg-background flex min-h-0 min-w-0 flex-col">
            {selectedFile ? (
              <>
                <div className="border-border/70 bg-background/90 shrink-0 border-b px-3 py-2.5 backdrop-blur sm:px-4 sm:py-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex min-w-0 items-center gap-2">
                      ...
                      {hasUnsavedChanges && (
                        <span className="rounded-full bg-amber-500/12 px-1.5 py-0.5 text-[10px] font-medium text-amber-700 dark:text-amber-300">
                ...
                <div className="min-h-0 flex-1 p-2 sm:p-3">
                  <div className="bg-muted/15 border-border/70 flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border shadow-inner">
                    {isMarkdownDocument ? (
                      <div
                        data-color-mode={theme}
                        className="workspace-md-editor flex min-h-0 flex-1 flex-col"
                      >
                        <MDEditor
                          value={content}
                          ...
                          textareaProps={{
                            placeholder: t(
                              "pages.agent.manage.workspaceFiles.placeholder",
                            ),
        ...
        <DialogFooter className="border-border/70 bg-background/95 shrink-0 border-t px-3 py-2 backdrop-blur sm:px-4 sm:py-2.5">
