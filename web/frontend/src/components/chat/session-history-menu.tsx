...
export function SessionHistoryMenu({
  ...
  if (loadError) {
    return (
      <span className='text-destructive text-xs'>{loadErrorMessage}</span>
    )
  }
  ...
  return (
    <div className='flex items-center gap-1'>
      {sessions.map((session) => (
        ...
        >
          <button
            className='flex flex-col items-start gap-0.5 min-w-0 flex-1 cursor-pointer'
          ...
          <div className='flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity'>
            ...
            <Button
              ...
              onClick={(e) => {
                ...
                if (
                  window.confirm(
                    t('chat.confirmDelete', { defaultValue: 'Delete this chat?' })
                  )
                ) {
